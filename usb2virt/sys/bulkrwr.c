...
--*/
{
    ...
    TraceEvents(TRACE_LEVEL_VERBOSE, DBG_READ, "--> BulkIoRead\n");
    ...
    WdfRequestSetCompletionRoutine(
                            ...
                            pipe);
    //
    // Send the request asynchronously on the read pipe.
    //
...
Exit:
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_READ, "<-- BulkIoRead\n");
...
--*/
{
    ...
    TraceEvents(TRACE_LEVEL_VERBOSE, DBG_WRITE, "--> BulkIoWrite\n");
    ...
    //
    // Send the request asynchronously to the bulk out pipe.
    //
    if (WdfRequestSend(Request, WdfUsbTargetPipeGetIoTarget(pipe), WDF_NO_SEND_OPTIONS) == FALSE) {
        //
        // Framework couldn't send the request for some reason.
